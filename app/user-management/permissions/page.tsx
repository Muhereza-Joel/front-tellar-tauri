...
const RoleListSkeleton = () => (
  <div className="p-2 space-y-2 animate-pulse">
    {[1, 2, 3, 4].map((i) => (
      <div
        key={i}
        className="h-10 bg-zinc-100 dark:bg-zinc-900 rounded-md w-full"
      />
    ))}
  </div>
);
...
const ResourceCardSkeleton = () => (
  <div className="bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-md shadow-sm overflow-hidden animate-pulse">
    ...
    <div className="p-4 grid grid-cols-2 gap-3">
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="h-8 bg-zinc-50 dark:bg-zinc-900/50 rounded-md"
        />
      ))}
    </div>
  </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-2">
        ...
        {canCreatePermission && (
          <aside className="lg:col-span-3">
            <div className="bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-sm overflow-hidden sticky top-0">
                  ...
                  {roles.map((role) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-md transition-all text-sm font-bold ${
        ...
        >
          <header className="flex flex-col sm:flex-row items-center justify-between bg-white dark:bg-black p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm gap-4">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-zinc-50 dark:bg-zinc-900 border border-transparent focus:border-blue-500 dark:focus:border-blue-600/50 rounded-md text-sm outline-none transition-all text-zinc-900 dark:text-zinc-100 focus:ring-1 focus:ring-blue-500"
                value={searchTerm}
            ...
            {canUpdatePermission && (
              <button
                ...
                className={`w-full sm:w-auto px-8 py-2.5 rounded-md text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-sm active:scale-[0.99] ${
                  isRootAdmin
          ...
          {isRootAdmin && selectedRoleId && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-md p-3 text-xs text-amber-700 dark:text-amber-400 flex items-center gap-2">
              <Shield size={14} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loading && filteredResources.length === 0
              ...
              : filteredResources.map((resource) => (
                  <div
                    key={resource.name}
                    className={`bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-md shadow-sm h-fit overflow-hidden transition-opacity ${
                      loading || isRootAdmin
                    ...
                    <div className="p-2 grid grid-cols-2 gap-1">
                      {resource.actions.map((action) => {
                        ...
                        return (
                          <label
                            key={permName}
                            className={`flex items-center gap-2 p-2 rounded-md cursor-pointer group transition-all border ${
                              isChecked
                          ...
                          >
                            <div className="relative flex items-center justify-center">
                              <input
                                type="checkbox"
                                className="peer appearance-none w-3.5 h-3.5 border border-zinc-300 dark:border-zinc-700 rounded-sm checked:bg-blue-600 checked:border-blue-600 transition-all cursor-pointer bg-transparent"
                                checked={isChecked}
          ...
          {!loading && filteredResources.length === 0 && (
            <div className="text-center py-20 bg-white dark:bg-black rounded-lg border border-dashed border-zinc-300 dark:border-zinc-800">
              <Shield
                size={40}
